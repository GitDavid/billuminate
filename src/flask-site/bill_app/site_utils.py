...
def apply_read_time(X):
    ...
    # Calculate the cumulative reading time with sentences
    # ranked using predict_proba
...
def create_read_time_slider(X, read_time=None):
    ...
    if not read_time_slider['current']:
        read_time_slider['current'] = X[X.prediction ==
                                        1]['time_cumulative'].max()
    read_time_slider['current'] = int(
        np.ceil(float(read_time_slider['current'])))
